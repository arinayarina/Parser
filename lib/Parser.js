...
class Parser {
    ...
    cursiveText(line) {
        ...
        while (line[0] !== line[i]) { // до этого объявили i = 1 - почему?
            // или i - это конец вложения?
            if (PatternsReg.Strong.reg.test(line.slice(i))) {
                if (text !== "") {
                    node.addChild(new Tree.Node("text", {text: text}));
                    text = "";
                }
                // вложенные сущности лучше доверить определять парсеру
            ...
            if (reg.test(line[i])) {
                ...
                // код ниже дублируется в трех местах, мб это вынести в отдельную функцию?
                // или доверить парсеру разбираться с особыми символами
                if (line[i] === "\\" && specialSymbols.indexOf(line[i + 1]) !== -1) {
                    node.addChild(new Tree.Node("specialSymbol", {symbol: line[i + 1]}));
                    i = i + 2;
                } else {
    ...
    parseHeader(line) {
        ...
        console.log(line.slice(words[0].length + 1));
        let node = new Tree.Node("Header");
        console.log("!", node);
        node.addToken({level: words[0].length});

        this.parseText(line.slice(words[0].length + 1), node);
    ...
    parseImage(img) {
        ...
        let a = img.indexOf("]");
        let b = img.indexOf(")");
        let alt = img.slice(2, a);
        let link = img.slice(a + 2, b);
        let node = new Tree.ImageNode("Image", words[0].length, alt, link);
        return node;
    }
